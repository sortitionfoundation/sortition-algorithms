...
def find_distribution_leximin(
    ...
    # cap initial committees at 100 to speedup
    ...
    # ==========================================
    # WHITEBOARD VALIDATION CHECKS
    # ==========================================
    total_prob = sum(probabilities_normalised)
    min_prob = min(fixed_probabilities.values()) if fixed_probabilities else 0.0
    
    print("\n" + "="*50)
    print(f"WHITEBOARD CHECKS (Dataset size: {people.count})")
    print("="*50)
    
    # Check 1: Do probabilities add to 1?
    print(f"1. Do probabilities add to 1? -> {total_prob:.6f} ", end="")
    if abs(total_prob - 1.0) < 1e-5:
        print("(✅ PASS)")
    else:
        print("(❌ FAIL)")
        
    # Check 2: The exact objective (lowest selection probability)
    print(f"2. Objective (Lowest Selection Prob) -> {min_prob:.6f}")
    print("="*50)

    # ==========================================
    # STRIP PLOTS & CSV GENERATION
    # ==========================================
    try:
        import csv
        import matplotlib.pyplot as plt

        dataset_size = people.count
        print(f"\nGenerating CSV and Strip Plot for dataset size {dataset_size}...")

        # save to CSV
        csv_filename = f"individual_probs_{dataset_size}.csv"
        with open(csv_filename, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["Agent_ID", "Selection_Probability"])
            for agent_id, prob in fixed_probabilities.items():
                writer.writerow([agent_id, f"{prob:.6f}"])

        # generate Strip Plot
        prob_values = list(fixed_probabilities.values())
        plt.figure(figsize=(10, 3))
        plt.scatter(prob_values, [0] * len(prob_values), alpha=0.3, color='blue', edgecolors='none')
        plt.title(f'Strip Plot of Selection Probabilities (N={dataset_size})', fontsize=14)
        plt.xlabel('Probability of being selected', fontsize=12)
        plt.yticks([])
        ...
        img_filename = f"strip_plot_{dataset_size}.png"
        plt.savefig(img_filename, dpi=300)
        plt.close() # Close figure to save memory
        print(f"✅ Saved {csv_filename} and {img_filename}!\n")
    except Exception as e:
        print(f"⚠️ Could not generate plot/csv: {e}\n")
