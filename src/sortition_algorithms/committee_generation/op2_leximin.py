...
# Option 2: Fixing the "Batch" Heuristic
...
def _run_leximin_heuristic_for_additional_committees(
    ...
    """Run heuristic to find additional committees to reduce outer loop calls."""
    counter = 0
    new_set = None

    for _ in range(10): # Try to harvest up to 10 extra committees
        if new_set is not None:
            for agent_id in new_set:
                # Penalize agents in the last found committee so it finds different ones
                agent_weights[agent_id] *= (upper / value) if value > EPS else 0.5

        sum_weights = sum(agent_weights.values())
        if sum_weights < EPS:
            break
        for agent_id in agent_weights:
            agent_weights[agent_id] /= sum_weights
        upper /= sum_weights

        new_committee_solver.set_objective(
            solver_sum(agent_weights[agent_id] * agent_vars[agent_id] for agent_id in people),
            SolverSense.MAXIMIZE,
        )
        new_committee_solver.optimize()
        new_set = ilp_results_to_committee(new_committee_solver, agent_vars)
        value = sum(agent_weights[agent_id] for agent_id in new_set)
...
def _run_leximin_column_generation_loop(
    ...
    while True:
        ...
        # --- NEW HEURISTIC TRICK INJECTED HERE ---
        counter = _run_leximin_heuristic_for_additional_committees(
        ...
        if counter > 0:
            logger.debug(f"Heuristic added {counter} extra committees.")
