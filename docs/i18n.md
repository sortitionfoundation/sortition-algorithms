...
For simple exceptions, check if they have `error_code` and `error_params` attributes (future enhancement):
...
try:
    ...
    # For now, just display the English message
    print(str(e))

    # Future: when simple errors gain error_code support
...
- Error codes for simple exceptions (BadDataError, SelectionError, etc.)
