...
import csv
import argparse
import logging  # <-- NEW: Imported the logging module!
...
try:
    from sortition_algorithms.committee_generation.leximin import find_distribution_leximin
except ImportError as e:
    print(f"CRITICAL ERROR: Could not locate leximin.py. Details: {e}")
    exit(1)
...
def leximin_probabilities(instance: Instance, backend: str = "mip") -> ProbAllocation:
    portfolio, output_probs, _ = find_distribution_leximin(
        instance.categories, 
        instance.agents, 
        instance.k, 
        [], 
...
def analyze_instance(instance_name: str, instance: Instance, backend: str, num_runs: int = 3):
    Path("analysis").mkdir(exist_ok=True)
    print(f"Running analysis with {backend} backend ({num_runs} runs)...")
    
    timings = []
    alloc = None
    for i in range(num_runs):
        start_time = time.time()
        alloc = leximin_probabilities(instance, backend)
    ...
    log_path = Path("analysis") / f"{instance_name}_{instance.k}_stats.txt"
    ...
    plt.scatter(probs, y_vals, s=18, alpha=0.55, color="tab:blue", label="Leximin")
    ...
    plt.yticks([0], ["Leximin"])
    plt.grid(axis="x", alpha=0.25)
    plt.ylim(-0.6, 0.6)
    plt.title(f"Marginal Selection Probabilities: {instance_name} (n={n}, k={instance.k})")
    plt.xlabel("Selection Probability")
    plt.legend(loc="upper right")
    
    output_path = Path("analysis") / f"{instance_name}_{instance.k}_plot.pdf"
...
if __name__ == '__main__':
    # --- NEW: Turn on the "chatty" debug logs! ---
    # To make it less spammy later, change logging.DEBUG to logging.INFO
